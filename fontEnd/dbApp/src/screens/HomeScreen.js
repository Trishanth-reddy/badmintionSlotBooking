...
const HomeScreen = ({ navigation }) => {
  ...
  const loadHomeData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      if (bookingsResponse.data.success) setUpcomingBookings(bookingsResponse.data.data);
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Players Wanted / Find Games Section */}
        <View style={styles.joinSection}>
          <Text style={styles.sectionTitle}>Players Wanted</Text>
          <TouchableOpacity style={styles.joinBanner} onPress={navigateToOpenMatches} activeOpacity={0.8}>
            <LinearGradient colors={['#8b5cf6', '#ec4899']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.bannerGradient}>
              <View>
                <Text style={styles.bannerTitle}>Find a Game</Text>
                <Text style={styles.bannerSubtitle}>Join players at local courts</Text>
              </View>
              <MaterialIcons name="group-add" size={40} color="#fff" style={{ opacity: 0.8 }} />
            </LinearGradient>
          </TouchableOpacity>
        </View>
        ...
        {/* Membership Banner */}
        <View style={styles.section}>
          <LinearGradient colors={['#ec4899', '#f43f5e']} style={styles.bannerCard}>
            <View style={styles.bannerContent}>
              <View>
                <Text style={styles.bannerBadge}>SPECIAL OFFER</Text>
                <Text style={styles.bannerTitle}>25% Off Yearly Plans</Text>
                <Text style={styles.bannerSubtitle}>Limited time offer</Text>
              </View>
              <Text style={styles.bannerEmoji}>🎉</Text>
            </View>
            <TouchableOpacity style={styles.bannerButton} onPress={navigateToSubscriptionPlans}>
        ...
        <View style={styles.section}>
          ...
          {upcomingBookings.length > 0 ? (
            ...
            <View style={styles.emptyState}>
              ...
              <TouchableOpacity style={styles.emptyStateButton} onPress={navigateToBookingFlow}>
...
const styles = StyleSheet.create({
  ...
  profileIcon: { width: 50, height: 50, borderRadius: 25, backgroundColor: 'rgba(255, 255, 255, 0.2)', justifyContent: 'center', alignItems: 'center' },
  scrollContent: { paddingBottom: 30 },
  section: { paddingHorizontal: 20, marginTop: 25 },
  sectionTitle: { fontSize: 20, fontWeight: 'bold', color: '#1f2937', marginBottom: 15 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  viewAllLink: { fontSize: 14, color: '#8b5cf6', fontWeight: '600' },
  quickActionsGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: 12 },
  quickActionCard: { width: (width - 52) / 2, borderRadius: 15, overflow: 'hidden', elevation: 5 },
  quickActionGradient: { paddingVertical: 20, alignItems: 'center' },
  quickActionLabel: { color: '#fff', fontSize: 14, fontWeight: '600', marginTop: 8 },
  
  // Players Wanted Section
  joinSection: { paddingHorizontal: 20, marginTop: 25 },
  joinBanner: { borderRadius: 15, overflow: 'hidden', elevation: 4 },
  ...
  bannerSubtitle: { color: '#fff', fontSize: 14, opacity: 0.9, marginTop: 4 },

  bannerCard: { borderRadius: 20, padding: 20, elevation: 6 },
  bannerContent: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
  bannerBadge: { fontSize: 12, fontWeight: 'bold', color: 'rgba(255, 255, 255, 0.8)' },
  bannerTitle: { fontSize: 22, fontWeight: 'bold', color: '#fff' },
  bannerEmoji: { fontSize: 48 },
  bannerButton: { backgroundColor: 'rgba(255, 255, 255, 0.2)', paddingVertical: 10, borderRadius: 10, alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.3)' },
  bannerButtonText: { color: '#fff', fontWeight: '600' },
  bookingsList: { gap: 12 },
  bookingCard: { flexDirection: 'row', backgroundColor: '#fff', borderRadius: 15, padding: 15, alignItems: 'center', elevation: 3 },
  bookingCardLeft: { width: 50, height: 50, borderRadius: 12, backgroundColor: '#f3e8ff', justifyContent: 'center', alignItems: 'center', marginRight: 15 },
  bookingCardMiddle: { flex: 1 },
  bookingCourt: { fontSize: 15, fontWeight: 'bold' },
  bookingTime: { fontSize: 13, color: '#8b5cf6', fontWeight: '500' },
  statusBadge: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 8 },
  statusConfirmed: { backgroundColor: '#dcfce7' },
  statusPending: { backgroundColor: '#fef08a' },
  statusText: { fontSize: 12, fontWeight: '600' },
  emptyState: { alignItems: 'center', paddingVertical: 40, backgroundColor: '#fff', borderRadius: 15 },
  emptyStateButton: { backgroundColor: '#8b5cf6', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 10, marginTop: 15 },
  emptyStateButtonText: { color: '#fff', fontWeight: '600' },
  statsContainer: { flexDirection: 'row', justifyContent: 'space-between', gap: 12 },
  statCard: { flex: 1, backgroundColor: '#fff', borderRadius: 15, padding: 16, alignItems: 'center', elevation: 3 },
  statNumber: { fontSize: 28, fontWeight: 'bold', color: '#8b5cf6' },
  statLabel: { fontSize: 12, color: '#6b7280', marginTop: 8, textAlign: 'center' },
});
